...
def predict(data,date,sess,std,directory,n_steps,n_inputs,prediction_window = 30):
    ...
    with tf.Session() as sess:
        ...
        predict_accum = std.transform(data[:n_steps]).tolist()
    ...
    return std.inverse_transform(np.array(predict_accum[-prediction_window:]).reshape(-1,1)),data[-prediction_window:],date[-prediction_window:]